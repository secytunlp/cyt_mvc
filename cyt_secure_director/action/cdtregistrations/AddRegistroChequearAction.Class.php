...
class AddRegistroChequearAction extends CdtAction{
	...
	public function execute(){
		...
		try{
			...
			if (empty($oDocente)) {
				$valido=false;
				$mensajes=CYT_SECURE_MSG_REGISTRATION_DIRECTOR_CONTROL_ERROR;
			}
			else $mensajes=CYT_SECURE_MSG_REGISTRATION_DIRECTOR_CONTROL_1.' '.$oDocente->getDs_apellido().', '.$oDocente->getDs_nombre().' '.CYT_SECURE_MSG_REGISTRATION_DIRECTOR_CONTROL_2;
