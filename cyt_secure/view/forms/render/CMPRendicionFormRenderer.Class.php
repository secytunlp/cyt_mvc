...
class CMPRendicionFormRenderer extends DefaultFormRenderer {
	...
	protected function renderFieldset(CMPForm $form, XTemplate $xtpl){
		...
		foreach ($form->getFieldsets() as $fieldset) {
			...
			$fieldCalle = $fields['observaciones'];		
			...
			$xtpl->parse("main.observaciones");
			
			$hiddens = $form->getHiddens();
			
			$xtpl->assign("value", CYT_LBL_RENDICION_RENDICION );
			$xtpl->assign("required", "*" );
			$xtpl->parse("main.rendicion.label");
			$xtpl->assign("actionFile", "doAction?action=add_file_session_rendicion&solicitud_oid=".$hiddens['solicitud.oid']->getInputValue() );
			$xtpl->parse("main.rendicion.input");
			$xtpl->assign("display", 'block');
			
			
			
			$hiddenDs_archivo = $hiddens['rendicion'];	
				
			if ($hiddenDs_archivo->getInputValue()) {
				$xtpl->assign("rendicion_cargado", '<span style="color:#009900; font-weight:bold">'.CYT_MSG_FILE_UPLOAD_EXITO.$hiddenDs_archivo->getInputValue().'</span>');
			}
			$xtpl->parse("main.rendicion");
			
			$xtpl->assign("value", CYT_LBL_RENDICION_INFORME );
			$xtpl->assign("required", "*" );
			$xtpl->parse("main.informe.label");
			$xtpl->assign("actionFile", "doAction?action=add_file_session_rendicion&solicitud_oid=".$hiddens['solicitud.oid']->getInputValue() );
			$xtpl->parse("main.informe.input");
			$xtpl->assign("display", 'block');
			
			
			
			$hiddenDs_archivo = $hiddens['informe'];	
				
			if ($hiddenDs_archivo->getInputValue()) {
				$xtpl->assign("informe_cargado", '<span style="color:#009900; font-weight:bold">'.CYT_MSG_FILE_UPLOAD_EXITO.$hiddenDs_archivo->getInputValue().'</span>');
			}
			$xtpl->parse("main.informe");
			...
			$xtpl->assign("required", "*" );
			$xtpl->parse("main.constancia.label");
			$xtpl->assign("actionFile", "doAction?action=add_file_session_rendicion&solicitud_oid=".$hiddens['solicitud.oid']->getInputValue() );
			$xtpl->parse("main.constancia.input");
			$xtpl->assign("display", $displayConstancia);
			
			
			
			$hiddenDs_archivo = $hiddens['constancia'];	
				
			if ($hiddenDs_archivo->getInputValue()) {
				$xtpl->assign("constancia_cargado", '<span style="color:#009900; font-weight:bold">'.CYT_MSG_FILE_UPLOAD_EXITO.$hiddenDs_archivo->getInputValue().'</span>');
			}
			$xtpl->parse("main.constancia");
