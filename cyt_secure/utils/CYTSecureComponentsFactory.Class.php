...
class CYTSecureComponentsFactory {
	...
	public static function getFindTituloWithAdd(Titulo $titulo, $required_msg="", $inputId='titulo_oid', $inputName='titulo.oid', $fCallback="titulo_change") {

		$oFindEntity = self::getFindTitulo($titulo, $required_msg, $inputId, $inputName, $fCallback);
	...
	public static function getFindTituloPosgradoWithAdd(Titulo $titulo, $required_msg="", $inputId='titulo_oid', $inputName='titulo.oid', $fCallback="titulo_change") {

		$oFindEntity = self::getFindTituloPosgrado($titulo, $required_msg, $inputId, $inputName, $fCallback);
		$oFindEntity->getInput()->setAddEntityAction("add_titulo_popup_init");
		//$oFindEntity->getInput()->setTitleAddEntity("Agregar nueva localidad");
		return $oFindEntity;
	}
	...
	public static function getFindRendicion(Rendicion $rendicion, $label, $required_msg="", $inputId='rendicion_oid', $inputName='rendicion.oid', $fCallback="rendicion_change") {
		...
		$findEntityInput = FieldBuilder::buildFieldFindEntity ($rendicion->getOid(), $label, $inputId, $inputName, self::getAutocompleteRendicion($rendicion), get_class(CYTSecureManagerFactory::getRendicionManager()), "CMPRendicionPopupGrid" , $required_msg );
		$findEntityInput->getInput()->setFunctionCallback($fCallback);		
